...
public class TestRunDataAssert extends AbstractAssert<TestRunDataAssert, TestRunData> {
	...
	public TestRunDataAssert hasFailedTest(TestClassName testClass, String testMethod) {
	...
	public TestRunDataAssert hasErroredTest(TestClassName testClass, String testMethod) {
	...
	final private TestFailure hasUnsuccessfulTest(TestClassName testClass, String testMethod,
		Class<? extends Throwable> errorClass, int status) {
		return hasUnsuccessfulTest(testClass, testMethod, errorClass, status, null, null);
	}
	...
	private static String messagePrefix(TestClassName testClass, String testMethod) {
		return String.format("Expecting test:\n%s\nin class:\n%s\n", testMethod, testClass.fqName());
	}

	private static String messagePrefix(TestClassName testClass) {
	...
	public TestRunDataAssert hasSuccessfulTest(TestClassName testClass, String testMethod) {
		messagePrefix = messagePrefix(testClass, testMethod);
	...
	public TestRunDataAssert hasSkippedTest(TestClassName testClass, String reason) {
		messagePrefix = messagePrefix(testClass);
		return hasAbortedOrSkippedContainer(nameOf(testClass), "Skipped: " + reason);
	}

	public TestRunDataAssert hasSkippedTest(TestClassName testClass, String testMethod, String reason) {
		messagePrefix = messagePrefix(testClass, testMethod);
		return hasAbortedOrSkippedTest(nameOf(testClass, testMethod), "Skipped: " + reason);
	}

	public TestRunDataAssert hasAbortedTest(TestClassName testClass, Throwable cause) {
		messagePrefix = messagePrefix(testClass);
		return hasAbortedOrSkippedContainer(nameOf(testClass), firstLine(cause.toString()));
	}

	public TestRunDataAssert hasAbortedTest(TestClassName testClass, String testMethod, Throwable cause) {
		messagePrefix = messagePrefix(testClass, testMethod);
